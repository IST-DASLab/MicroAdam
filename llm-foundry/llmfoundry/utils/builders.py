...
from composer.loggers import (InMemoryLogger, LoggerDestination, MLFlowLogger, TensorboardLogger, WandBLogger)
...
def build_logger(name: str, kwargs: Dict[str, Any]) -> LoggerDestination:
    if name == 'wandb':
        if 'job_type' in kwargs:
            kwargs['init_kwargs'] = dict(job_type=kwargs['job_type'])
            del kwargs['job_type']
...
def _extract_param_groups(
    ...
    if 'disable_grad' in optimizer_config.keys():
        # print(f'\t@@@@@[_extract_param_groups] IF-ELSE #1: IF-BRANCH')
    ...
    # else:
    #     print(f'\t@@@@@[_extract_param_groups] IF-ELSE #1: ELSE-BRANCH')

    param_groups_config = optimizer_config.pop('param_groups', None)
    if param_groups_config is not None:
        # print(f'\t@@@@@[_extract_param_groups] IF-ELSE #2: IF-BRANCH')
        params = []
        ...
        return params
    # else:
    #     print(f'\t@@@@@[_extract_param_groups] IF-ELSE #2: ELSE-BRANCH')
...
def build_optimizer(model: torch.nn.Module, name: str,
    ...
    if name in ['galoreadamwef']:
        ...
        params = [
            {'params': regular_params, 'should_apply_galore': False},
            {'params': galore_params, 'should_apply_galore': True}
        ]
    else:
    ...
    custom_cfg = optimizer_config[name]
    if custom_cfg is None:
        custom_cfg = dict()
    cfg = {**optimizer_config['defaults'], **custom_cfg}

    if name == 'decoupled_adamw':
        return DecoupledAdamW(params, **cfg)
    elif name == 'adamw8b':
        ...
        cfg['beta1'], cfg['beta2'] = cfg['betas']
        del cfg['betas']
        return GaLoreAdamWEF(params, **cfg)
    else:
        raise ValueError(f'Not sure how to build optimizer: {name}')
